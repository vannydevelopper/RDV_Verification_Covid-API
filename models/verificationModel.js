...
const checkdroit = async (column, value) => {
  try {

    var sqlQuery = `SELECT users.POINT_ENTREE_ID,users.USER_PROFILE_ID,users_profile.GESTION_RENDEZ_VOUS,users_profile.VALIDATION_RESULTAT,users_profile.GESTION_FILS_ATTENTE_LABO FROM users LEFT JOIN users_profile ON users.USER_PROFILE_ID=users_profile.USER_PROFILE_ID  WHERE  ${column}= ?`
    return query(sqlQuery, [value]);
